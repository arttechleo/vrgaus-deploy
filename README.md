...
- Real-time 3D Gaussian Splat rendering
...
2. If using a VR headset (like Meta Quest), ensure you're using a WebXR-compatible browser
3. Click the VR/AR button to enter immersive mode
4. The demo will load a sample 3D model (bonsai tree) that you can interact with in VR/AR
...
2. Serve the directory using any static file server (e.g., `python -m http.server` or `npx serve`)
3. Access through a WebXR-compatible browser

## Credits

- Based on the [Gaussian Splats 3D](https://github.com/mkkellogg/GaussianSplats3D) library by Mark Kellogg
- Uses [Three.js](https://threejs.org/) for 3D rendering

# Gaussian Splats 3D - AR/VR Viewer

A web-based viewer for 3D Gaussian Splatting models with AR/VR support.

## Features

- View 3D Gaussian Splatting models in AR/VR mode
- Optimized for Quest 3 and other WebXR-compatible devices
- Fast loading and processing of splat models

## Usage

1. Open the application in a WebXR-compatible browser (like the Meta Quest Browser)
2. Click the AR/VR button to enter immersive mode
3. Interact with the 3D model in your environment

## Deployment

This project is designed to be deployed on Render.com or similar static site hosting services.
